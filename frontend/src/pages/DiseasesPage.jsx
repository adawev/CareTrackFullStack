...
export default function DiseasesPage() {
  ...
  async function handleSave() {
    try {
      ...
      toast.error(err.response?.data?.message || "Error saving disease");
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold text-gray-900">Diseases</h2>
        {isAdmin && (
          <Button size="sm" onClick={openCreate}>
            <Plus size={16} className="mr-2" /> Add Disease
          </Button>
        )}
      </div>
            ...
            {diseases.length === 0 ? (
              ...
              diseases.map((d) => (
                <TableRow key={d.id}>
                  ...
                  <TableCell>
                    <Badge variant={SEVERITY_COLORS[d.severity] || "outline"}>{d.severity}</Badge>
                  </TableCell>
                  <TableCell>
                    <Badge variant={STATUS_COLORS[d.status] || "outline"}>{d.status}</Badge>
                  </TableCell>
